...
fn main(mut request: Request) -> Result<Response, Error> {
    let mut backend_response: Option<Response> = None;

    // Where is the application running?
    let pop = get_pop(&request);
    ...
    for origin in POP_ORIGIN.get(pop.as_str()).unwrap_or(&[US_WEST, US_EAST, EU_CENTRAL]) {
        let mut backend_request = request.clone_without_body();
        ...
        backend_request.set_header(header::HOST, &host);

        // Set the AWS V4 authentication headers
        set_authentication_headers(&mut backend_request, &origin);

        let mut response = backend_request.send(origin.backend_name)?;

        if response.get_status().is_success() {
            // Set a response header indicating the origin that we used
            response.set_header("X-B2-Host", &host);
    ...
    match backend_response {
        ...
        None => Ok(Response::new().with_status(StatusCode::NOT_FOUND)),
    }
}
